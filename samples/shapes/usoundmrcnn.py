...
class VidFrameDataset(utils.Dataset):
    """Generates the shapes synthetic dataset. The dataset consists of simple
    shapes (triangles, squares, circles) placed randomly on a blank surface.
    The images are generated on the fly. No file access required.
    """
    ...
    def load_shapes(self, count, height, width):
        ...
        """
        # Add classes
        self.add_class("shapes", 1, "ellipse")
#         self.add_class("shapes", 2, "circle")
#         self.add_class("shapes", 3, "triangle")
        ...
        if(count != -1):
            for i in range(2550):
                temp_img = cv2.imread(pdir + 'Data/Img_Train/' + str(i) + '.png', -1)
#                 print('0:', temp_img.shape[0], '1:', temp_img.shape[1])
                self.add_image("shapes", image_id = i, path = pdir + 'Data/Img_Train/' + 
                               str(i) + '.png', width = temp_img.shape[1], height = temp_img.shape[0],
...
#                 cv2.destroyAllWindows()
            ...
            for i in range(2550, 2694):
                temp_img = cv2.imread(pdir + 'Data/Img_Val/' + str(i) + '.png', -1)
#                 print('0:', temp_img.shape[0], '1:', temp_img.shape[1])
                self.add_image("shapes", image_id = i, path = pdir + 'Data/Img_Val/' + 
                               str(i) + '.png', width = temp_img.shape[1], height = temp_img.shape[0],
                              bg_color = np.array([0, 0, 0]))
#                 print(i)
...
#             image = self.draw_shape(image, shape, dims, color)
        ...
        if(image_id < 2550):
            image = cv2.imread(pdir + 'Data/Img_Train/' + str(image_id) + '.png', -1)
        else:
            image = cv2.imread(pdir + 'Data/Img_Val/' + str(image_id) + '.png', -1)
        return image
    ...
    def load_mask(self, image_id):
        ...
        if(image_id < 2550):
            tmp_mask = cv2.imread(pdir + 'Data/Masks_Train/' + str(image_id) + '.png', -1)
        elif(image_id >= 2550 and image_id < 2694):
            tmp_mask = cv2.imread(pdir + 'Data/Masks_Val/' + str(image_id) + '.png', -1)
...
model_path = os.path.join(pdir + 'Models_MG/', "mask_rcnn_shapes_F.h5")
...
# class InferenceConfig(VidConfig):
#     GPU_COUNT = 1
#     IMAGES_PER_GPU = 1
#     USE_MINI_MASK = False
